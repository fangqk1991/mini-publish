...
class Project {
  ...
  _loadInfo() {
    ...
    assert.ok(!!dic && typeof dic === 'object')
    assert.ok(!!dic.remote && typeof dic.remote === 'object')
    ...
    if (Array.isArray(remoteInfo.exeBefore)) {
      this._exeBefore = remoteInfo.exeBefore.join('; ')
    }

    if (Array.isArray(remoteInfo.exeAfter)) {
      this._exeAfter = remoteInfo.exeAfter.join('; ')
    }
  }
